defmodule Darwin.ExToErl do
  ...
  def elixir_ast_to_erlang_abstract_code(ast) do
    {variables, new_ast} = extract_variables_from_elixir_ast_and_set_context_to_nil(ast)
    ...
    try do
      ...
      body
    after
      ModuleQueue.push(module_name)
    end
  end

  def elixir_module_source_to_erlang_abstract_code(module_name, module_body) do
    body_ast = Code.string_to_quoted!(module_body)
    elixir_module_ast_to_erlang_abstract_code(module_name, body_ast)
  end

  def elixir_module_source_to_erlang_source(module_name, module_body) do
    body_ast = Code.string_to_quoted!(module_body)
    abstract_code = elixir_module_ast_to_erlang_abstract_code(module_name, body_ast)
  ...
  defp extract_variables_from_elixir_ast_and_set_context_to_nil(ast) do
    {ast, variables} =
      Macro.postwalk(ast, [], fn ast_node, variables ->
        case ast_node do
          {name, meta, module} when is_atom(name) and is_atom(module) ->
            new_variable = {name, meta, nil}
            {new_variable, [new_variable | variables]}
